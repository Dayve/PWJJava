...
CREATE TABLE KATEGORIA (
	...
	CONSTRAINT KATEGORIA_PK PRIMARY KEY (ID_KAT)
);
...
--	PKT_PRC NUMBER(3),						-- Ranking? statystkyki sumaryczne?

	CONSTRAINT UZYTKOWNIK_PK PRIMARY KEY (ID_UZYT)
);

CREATE TABLE PYTANIA (
	ID_PYT NUMBER(8) GENERATED BY DEFAULT ON NULL AS IDENTITY,
	TRESC VARCHAR2(60) NOT NULL,
	ID_KAT NUMBER(6) CONSTRAINT FK_KATEGORIA_PYTANIA REFERENCES KATEGORIA(ID_KAT),

	CONSTRAINT PYTANIA_PK PRIMARY KEY (ID_PYT)
);

CREATE TABLE PYTANIE_ODPOWIEDZI (
	ID_PYT NUMBER(8) CONSTRAINT FK_ODP_PYTANIE REFERENCES PYTANIA(ID_PYT),
	ID_ODP NUMBER(10) CONSTRAINT FK_PYT_ODPOWIEDZ REFERENCES ODPOWIEDZI(ID_ODP)
);

CREATE TABLE TESTY (
	ID_TESTU NUMBER(6) GENERATED BY DEFAULT ON NULL AS IDENTITY,
	NAZWA_TESTU VARCHAR2(30) NOT NULL,
	ID_KAT NUMBER(6) CONSTRAINT FK_KATEGORIA_TESTU REFERENCES KATEGORIA(ID_KAT),

	CONSTRAINT TEST_PK PRIMARY KEY (ID_TESTU)
);
...
CREATE TABLE PROBA (
	ID_PROBY NUMBER(8) GENERATED BY DEFAULT ON NULL AS IDENTITY,
	WYNIK_TESTU NUMBER(3) NOT NULL,					-- Procentowy?
	ID_TESTU NUMBER(6) CONSTRAINT FK_PODEJSCIE_DO_TESTU REFERENCES TESTY(ID_TESTU),

	CONSTRAINT PROBA_PK PRIMARY KEY (ID_PROBY)
);

CREATE TABLE PODEJSCIA_UZYTKOWNIKA (					-- Można połączyć z PROBA
	ID_UZYT NUMBER(8) CONSTRAINT FK_UZYTKOWNIK_PROBA REFERENCES UZYTKOWNIK(ID_UZYT),
	ID_PROBY NUMBER(8) CONSTRAINT FK_ID_PROBY REFERENCES PROBA(ID_PROBY)
);
